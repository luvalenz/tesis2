...
def execute_pipleline(n_prototipes, window_size, step):
    ...
    call3 = 'python 3.tree_building lightcurves.R.txt {0} std weighted {1} {2}'.format(n_prototipes, window_size, step)
    model_name = 'sequence_tree_lightcurves.R.txt_{0}samples_semistdFalse_stdTrue_20levels_{1}_{2}.dill'.format(n_prototipes,
                                                                                                          window_size, step)
    call4 = 'python 4.ndcg_test {0} 100 20'.format(model_name)
